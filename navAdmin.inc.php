<nav class="navbarUser">
    <div class="logoNav" >
    <a href="index.php" class="logo"><img src="./assets/HOM.png" alt="logo"></a>
    </div>

    <div class="btnNav">
      <a class="btnsNav" href="adminIndex.php">Shop</a>
      <a class="btnsNav" href="ourTeamAdmin.php">Our team</a>
      <a class="btnsNav" href="myAdmin.php">My admin</a>
      <a class="btnsNav" href="addProduct.php">Add product</a>
    </div>
    
    <div class="restNav">
      <a href="logout.php" class="navbar__logout">Logout</a>
    </div>
    
</nav>